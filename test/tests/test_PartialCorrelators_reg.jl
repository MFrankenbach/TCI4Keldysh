@testset "PartialCorrelator_reg for formalism==MF" begin
    ...
    G_p_disc = TCI4Keldysh.PartialCorrelator_reg(T, "MF", Adisc, ωdisc, (ω_fer, ω_bos), [1 0; 0 1])
    ...
    G_p_disc = TCI4Keldysh.PartialCorrelator_reg(T, "MF", Adisc, ωdisc, (ω_fer, ω_bos), [1 0; 1 1])
    G_p_data = G_p_disc.(collect(1:length(ω_fer)), collect(1:length(ω_bos))')
    ...
    G_p_disc = TCI4Keldysh.PartialCorrelator_reg(T, "MF", Adisc, ωdisc, (ω_fer, ω_bos), [1 1; 1 -1])
    G_p_data = G_p_disc.(collect(1:length(ω_fer)), collect(1:length(ω_bos))')
    ...
    G_p_disc = TCI4Keldysh.PartialCorrelator_reg(T, "MF", Adisc, ωdisc, (ω_fer, ω_bos), [-1 1; 0 1])
    G_p_data = G_p_disc.(collect(1:length(ω_fer)), collect(1:length(ω_bos))')
    ...
    G_p_disc = TCI4Keldysh.PartialCorrelator_reg(T, "MF", Adisc, ωdisc, (ω_bos, ω_fer, ω_fer), [1 0 1; 1 1 1; -1 1 0])
