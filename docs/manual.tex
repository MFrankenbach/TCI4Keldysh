...
Further features include the computation of four-point vertices on dense, possibly nonlinear, grids and the computation of 2/3/4-point correlators, either
in QTT format or on a dense, linear grid.
\end{section}

\begin{section}{Getting started}
\label{sec:getting_started}
The code is exclusively written in Julia (version 1.10.3). To install all required packages,
...
Off-the-shelf partial spectral functions (PSFs) can be downloaded from OpenDataLMU \href{https://doi.org/10.5282/ubm/data.613}{here}.
Once you have suitable spectral data, set an environment variable \texttt{TCI4KELDYSH\_DATADIR} to
the parent directory of the directory containing your set of spectral functions.

If your PSFs come from an older version (before spring 2026) of MuNRG, you also have to export the environment variable \texttt{MPNRG\_VERSION} and set it to \texttt{old} (default is \texttt{new}).
One way to find out which version was used to generate your PSF data is to check the PSF `.mat` files have keys `Adisc` or `PSFinfo` (\textit{old} version) or simply a key `PSFdata` holding a bare array of spectral peaks (\textit{new} version).

With a single PSF data set, your \texttt{TCI4KELDYSH\_DATADIR} should be structured as follows:
\begin{verbatim}
    <\texttt{TCI4KELDYSH\_DATADIR}>/
    |
    --<dataset name>/
        |
        --<PSF folder name>/
        ...
        mpNRG.mat
\end{verbatim}
The \texttt{mpNRG.mat} file contains the physical and numerical (broadening etc.) parameters.
Only default broadening parameters and temperatures are read from these files.
...
The code can be run using input files by executing the command:
\begin{verbatim}
...
\texttt{jobtype}, which specifies the type of calculation. For example:
\begin{verbatim}
TCI4Keldysh BEGIN
...
export the environment variable \texttt{PWTCIDIR} in your SLURM script before running the calculation.
...
specified with $\texttt{iK}\in\{1,\ldots,15\}$ ($\texttt{iK}=16$ is zero). The corresponding four Keldysh indices
...
A \texttt{cutoff} of $10^{-6}$ was sufficient for tolerances of $\tau\geq10^{-4}$ in our calculations. Increasing \texttt{nlevel} speeds
...
\begin{section}{Contributing authors and acknowledgement}
This document was written by Markus Frankenbach. The \texttt{TCI4Keldysh} code was
written by Markus Frankenbach and Anxiang Ge in the group of Jan von Delft.
If you use this code in your work, please cite \cite{Frankenbach2025}. If your project heavily relies on the code, it's
authors may need to be coauthors of the paper. That being said, collaborations are very welcome.
\end{section}
