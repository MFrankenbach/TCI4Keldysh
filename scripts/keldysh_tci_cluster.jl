...
function FullCorrelator_jobs(args)
    println(" ==== COMPILE")
    time_compress_FullCorrelator_KF(2; R=3, tolerance=1.e-3)    
    println(" ==== RUN")
    run_nr = parse(Int, args[2])
...
function main(args)
    ...
    if occursin("corr", jobtype)
        FullCorrelator_jobs(args)
    elseif jobtype=="vertex"
        Γcore_jobs(args)
    else
