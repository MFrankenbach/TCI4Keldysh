...
function time_Γcore_sweep(
    ...
    beta=TCI4Keldysh.dir_to_T(PSFpath), tolerance=1.e-8, cache_center=0, serialize_tts=true
    )
    folder = "pwtcidata"
    channel = "t"
...
function main(args)
    ...
    #@assert nthreads==16 "Running with only $nthreads threads, should have 16"
    ...
    elseif run_nr>=1000
        ...
        time_Γcore_sweep(R:R, PSFpath, "R"; tolerance=tolerance, serialize_tts=true)
    else
