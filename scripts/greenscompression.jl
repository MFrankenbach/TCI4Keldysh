...
function compress_npoint(PSFpath::String; max_npoint=1, R=8,
    op_dict::Dict{Int, Vector{String}}=Dict(1=>["Q12","Q34"], 2=>["Q12", "F3", "F3dag"], 3=>["F1","F1dag", "F3", "F3dag"])) :: Tuple{Vector{Float64}, Vector{Float64}, Dict{Int, Array}}
    ...
    if haskey(op_dict, 1)
        K1ts    = [TCI4Keldysh.FullCorrelator_MF(PSFpath, op_dict[1]; T, flavor_idx=i, ωs_ext=(ωbos,), ωconvMat=ωconvMat_K1t, name="SIAM 2pG") for i in 1:2];
    ...
    if max_npoint >= 2 && haskey(op_dict, 2)
        Gs      = [TCI4Keldysh.FullCorrelator_MF(PSFpath, op_dict[2]; T, flavor_idx=i, ωs_ext=(ωbos,ωfer), ωconvMat=ωconvMat_K2′t, name="SIAM 3pG", is_compactAdisc=false) for i in 1:2]
    ...
    if max_npoint >= 3 && haskey(op_dict, 3)
        G3D     = TCI4Keldysh.FullCorrelator_MF(PSFpath*"/4pt/", op_dict[3]; T, flavor_idx=1, ωs_ext=(ωbos,ωfer,ωfer), ωconvMat=ωconvMat_t, name="SIAM 4pG", is_compactAdisc=false)
...
function compress_all_1pt(R::Int, tolerance::Float64)
    ...
    for opl in op_lists
        ...
        ombos, omfer, data_dict = compress_npoint(
            "data/SIAM_u=0.50/PSF_nz=2_conn_zavg/"; max_npoint=1, op_dict=op_dict, R=R
        )
    ...
    # store to json
    logJSON(gf_vec, "all_1pt_GF_R=$(R)_logtol=$(Int(round(-log10(tolerance))))")
end

function compress_all_2pt(R::Int, tolerance::Float64)
    op_lists = []
    ...
    for opl in op_lists
        ...
        op_dict = Dict(2=>opl)
        ombos, omfer, data_dict = compress_npoint(
            "data/SIAM_u=0.50/PSF_nz=2_conn_zavg/"; max_npoint=2, op_dict=op_dict, R=R
        )
    ...
    # store to json
    logJSON(gf_vec, "all_2pt_GF_R=$(R)_logtol=$(Int(round(-log10(tolerance))))")
end

function compress_all_3pt(R::Int, tolerance::Float64)
    ...
    for opl in op_lists
        ...
        op_dict = Dict(3=>opl)
        ombos, omfer, data_dict = compress_npoint(
            "data/SIAM_u=0.50/PSF_nz=2_conn_zavg/"; max_npoint=3, op_dict=op_dict, R=R
        )
    ...
    # store to json
    logJSON(gf_vec, "all_3pt_GF_R=$(R)_logtol=$(Int(round(-log10(tolerance))))")
end
...
"""
function compress_all_npoint_Rsweep(npt::Int, Rrange; tolerance=1e-6)
    for R in Rrange
        if npt==1
            compress_all_1pt(R, tolerance)
        elseif npt==2
            compress_all_2pt(R, tolerance)
        elseif npt==3
            compress_all_3pt(R, tolerance)
        else
...
"""
function compress_all_npoint_tolsweep(npt::Int, tolrange; R=8)
...
function plot_ranks_vs_R(npt::Int, Rrange, tolerance::Float64)
    # read in ranks
    worstcases = Int[]
    ranks = Vector{Int}[]
    for R in Rrange
        filename = "all_$(npt)pt_GF_R=$(R)_logtol=$(Int(round(-log10(tolerance)))).json" 
        # list of dicts
    ...
    scatter(xcoords, ycoords; color=:blue, label="TT rank", yscale=:log10, marker=:cross)
    scatter!(collect(Rrange), worstcases; color=:red, label="worst case", yscale=:log10)
    xlabel!("TT length")
    ylabel!("TT rank")
    title!("TCI ranks $(npt)pt-functions, tol=1e$(Int(round(log10(tolerance))))")
    savefig("ranks_vs_R_$(npt)pt_logtol=$(Int(round(-log10(tolerance)))).png")
end


function plot_ranks_vs_tol(npt::Int, tolrange, R::Int)
    # read in ranks
    worstcases = Int[]
    ranks = Vector{Int}[]
    for tol in tolrange
        filename = "all_$(npt)pt_GF_R=$(R)_logtol=$(Int(round(-log10(tol)))).json" 
        # list of dicts
        data = readJSON(filename)
        worst = maximum(TCI4Keldysh.worstcase_bonddim(convert.(Int, data[begin]["sitedims"])))
        push!(worstcases, worst)
    ...
    scatter(xcoords, ycoords; color=:blue, label="TT rank", yscale=:log10, xscale=:log10, marker=:cross)
    scatter!(collect(tolrange), worstcases; color=:red, label="worst case", yscale=:log10)
    xlabel!("TCI tolerance")
    ylabel!("TT rank")
    title!("TCI ranks $(npt)pt-functions, R=$R")
    savefig("ranks_vs_tol_$(npt)pt_R=$R.png")
end
...
# try_matopen("/Users/M.Frankenbach/tci4keldysh/data/SIAM_u=1.50/PSF_nz=4_conn_zavg/PSF_((Q13,Q24)).mat")
# try_matopen("/Users/M.Frankenbach/tci4keldysh/data/SIAM_u=0.50/PSF_nz=2_conn_zavg/PSF_((Q13,Q24)).mat")

# # 1 and 2pt
# compress_all_npoint_Rsweep(1, 4:12; tolerance=1e-6)
# compress_all_npoint_Rsweep(2, 4:12; tolerance=1e-6)
# compress_all_npoint_tolsweep(1, 10.0 .^ (-8.0:-3.0); R=8)
# compress_all_npoint_tolsweep(2, 10.0 .^ (-8.0:-3.0); R=8)


# 3pt
# compress_all_npoint_Rsweep(3, 4:8; tolerance=1e-5)
# compress_all_npoint_tolsweep(3, 10.0 .^ (-7.0:-3.0); R=8)

plot_ranks_vs_tol(1, 10.0 .^ (-8.0:-3.0), 8)
plot_ranks_vs_tol(2, 10.0 .^ (-8.0:-3.0), 8)
plot_ranks_vs_tol(3, 10.0 .^ (-7.0:-3.0), 8)