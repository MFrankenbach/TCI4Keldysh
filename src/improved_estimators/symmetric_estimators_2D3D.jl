...
function compute_Γcore_symmetric_estimator(
    ...
    for letts in letter_combinations#[2:end]
        println("letts: ", letts)
        Γcore_tmp      = TCI4Keldysh.FullCorrelator_MF(PSFpath, [letts[1]*op_labels[1], letts[2]*op_labels[2], letts[3]*op_labels[3], letts[4]*op_labels[4]]; flavor_idx, ωs_ext, ωconvMat);
